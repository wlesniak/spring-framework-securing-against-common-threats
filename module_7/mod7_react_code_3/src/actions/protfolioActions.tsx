...
export const addPortfolioTransactionCreator : ActionCreator<
    ...
    > = (loginAction: LoginAction) => {
        return async (dispatch :  ThunkDispatch<{},{},any>) => {
            const header = {'Authorization': 'Basic ' +window.btoa(loginAction.username+":"+loginAction.password)};
            const response = await axios.get(PORTFOLIO_SERVICE_DOMAIN+'/login/basic',
            {headers: header});
                dispatch(getPortfolioPositionsCreator());
        }
    }
